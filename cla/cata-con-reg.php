...
$eCodPagina        = $data->eCodDatoContacto                    ? $data->eCodDatoContacto                     : false;
...
if(!sizeof($errores))
{
    if(!$eCodPagina)
    {
        $query = "INSERT INTO CatDatosContacto
                    (
                        tContenido
    ...
    {
        $query = "UPDATE CatDatosContacto
                    SET
                        tContenido              =   $tContenido
                    WHERE eCodDatoContacto       =   $eCodPagina
                    ";
        $bTipo = 2;
    }
...
if(!sizeof($errores))
{
    $tDescripcion = "Se ha ".(($bTipo==1) ? 'insertado' : 'actualizado')." los datos de contacto ".sprintf("%07d",$eCodPagina);
